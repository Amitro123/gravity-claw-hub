...
import { Pause, Play, Search, Settings, Loader2 } from "lucide-react";
            ...
            <Card key={a.id} className="hover:border-primary/30 transition-colors">
              ...
              <CardContent className="space-y-3">
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div><span className="text-muted-foreground">Model:</span> <span>{a.model?.primary || "N/A"}</span></div>
                  <div><span className="text-muted-foreground">Uptime:</span> <span>N/A</span></div>
                  <div><span className="text-muted-foreground">Cost:</span> <span className="text-success">$0.00</span></div>
                  <div className="col-span-2"><span className="text-muted-foreground">Task:</span> <span>N/A</span></div>
                </div>
                <div className="flex gap-2 pt-1">
                  <Button variant="outline" size="sm" className="flex-1">
                    {status === "online" ? <><Pause className="h-3 w-3 mr-1" />Pause</> : <><Play className="h-3 w-3 mr-1" />Resume</>}
                  </Button>
